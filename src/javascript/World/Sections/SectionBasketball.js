...
export default class SectionBasketball {
    constructor(_options) {
        ...
        if (this.time) {
            ...
            console.warn('SectionBasketball: time parametresi verilmedi, animasyonlar çalışmayacak.');
        }
    }

    setModel() {
        if (!this.scene) {
            console.warn('SectionBasketball: scene parametresi verilmedi, model sahneye eklenmeyecek.');
            return;
        }
        ...
        loader.load('./models/SectionBasketball/base.glb', (gltf) => {
            ...
            this.model.position.set(-50, 14, .4);
            this.model.scale.set(.5,.5,.5);

            this.scene.add(this.model);

            this.model.rotation.x = Math.PI / 2; // Rotate the model to correct orientation
            this.model.rotation.y = .07; // Rotate the model to correct orientation
