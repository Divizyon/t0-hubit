...
export default class SectionYoungCenter {
    constructor(_options) {
        ...
        if (this.time) {
            ...
            console.warn('SectionYoungCenter: time parametresi verilmedi, animasyonlar çalışmayacak.');
        }
    }

    setModel() {
        if (!this.scene) {
            console.warn('SectionYoungCenter: scene parametresi verilmedi, model sahneye eklenmeyecek.');
            return;
        }

        const loader = new GLTFLoader();
        loader.load('./models/SectionYoungCenter/base.glb', (gltf) => {
            
            this.model = gltf.scene;
            this.model.position.set(40, -16, -1);
            this.model.scale.set(1, 1, 1);

            // Modelin üstü alt tarafta olduğundan düzeltmek için X ekseninde 180 derece döndürülüyor
            ...
            if (this.physics) {
                this.collisionBody = new CANNON.Body({
                    mass: 0,
                    position: new CANNON.Vec3(-20, -20, -2),
