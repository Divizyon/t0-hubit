...
export default class SectionDivision {
    constructor(_options) {
        ...
        if (this.time) {
            ...
            console.warn('SectionDivision: time parametresi verilmedi, animasyonlar çalışmayacak.');
        }
    }

    setModel() {
        if (!this.scene) {
            console.warn('SectionDivision: scene parametresi verilmedi, model sahneye eklenmeyecek.');
            return;
        }

        const loader = new GLTFLoader();
        loader.load('./models/SectionDivision/base.glb', (gltf) => {
            
            this.model = gltf.scene;
            this.model.position.set(-28, 4, 0);
            this.model.scale.set(.85, .85, .85);

            this.scene.add(this.model);


            this.model.rotation.z = 1.2;

          
            if (this.physics) {
                this.collisionBody = new CANNON.Body({
                    mass: 0,
                    position: new CANNON.Vec3(-28, 4, 1.5),
