...
export default class SectionBasketballCourt {
  ...
  _buildModel() 
  {
    const gltf = this.resources.items.BasketballCourt;
        const base = this.resources.items.Base;
      
        if (!gltf || !gltf.scene) {
          console.error('SectionBasketballCourt bina modeli bulunamadı');
          return;
        }
        ...
        model.rotation.set(Math.PI / 2, Math.PI / 5, this.rotateZ);
